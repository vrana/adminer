	...
	if ($_GET["ns"] !== "") {
		...
		if (!$tables_list) {
			...
			foreach ($tables_list as $name => $type) {
				...
				if ($view) {
					$displayedType = preg_match('~materialized~i', $type) ? lang('Materialized View') : lang('View');
					echo '<td colspan="6"><a href="' . h(ME) . "view=" . urlencode($name) . '" title="' . lang('Alter view') . '">' . $displayedType . '</a>';
