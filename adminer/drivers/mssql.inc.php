...
if (isset($_GET["mssql"])) {
	...
	function fields($table) {
		$comments = get_key_vals("SELECT objname, cast(value as varchar) as value 
...
WHERE o.schema_id = SCHEMA_ID(" . q(get_schema()) . ") AND o.type IN ('S', 'U', 'V') AND o.name = " . q($table)
			...
			$return[$row["name"]] = array(
				...
				"comment" => isset($comments[$row["name"]]) ? $comments[$row["name"]] : '',
			);
		}
		return $return;
	}
...
//			var_dump($val);die();
		...
		foreach ($comments as $key => $val) {
			...
			queries("EXEC sp_dropextendedproperty
