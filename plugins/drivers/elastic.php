...
if (isset($_GET["elastic"])) {
	...
	class Driver extends SqlDriver {
		...
		static function jushModule(): string {
			return <<<'JS'
jush.tr.elastic={json:/:(?= )/};jush.build_links2('elastic','https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-$key',/(\b)/,/(\b)/g,{'https://www.elastic.co/docs/api/doc/elasticsearch/':/(GET|POST|PUT|DELETE|HEAD)/,'search':/(_search)/,'count':/(_count)/,'index':/(_doc)/,'update':/(_update)/,'indices-put-mapping':/(_mapping)/,'indices-update-aliases':/(_aliases)/,'indices-get-alias':/(_alias)/,'indices-stats':/(_stats)/,});
JS;
		}
