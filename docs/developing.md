...
Driver plugins in [plugins/drivers/](/plugins/drivers/) are not compiled, so a driver which highlights its queries by an own JUSH module carries it inline in `Driver::jushModule()`.
The core prints the returned code in an inline `<script>` wrapped in `addEventListener('DOMContentLoaded', ...)` because `jush.js` is deferred and would otherwise run later.
The method is generated by [compile.php](/compile.php) from `externals/jush/modules/jush-<driver>.js`, minified by JsShrink and stored in a nowdoc - never edit it by hand, run `composer compile` after changing the module.
Drivers whose module is bundled in `jush.js` (the ones in [adminer/drivers/](/adminer/drivers/)) return an empty string.

The autocomplete of the query `<textarea>` is created by `Driver::jushAutocomplete()`, which returns an empty string in drivers whose queries are not SQL (e.g. Redis).
